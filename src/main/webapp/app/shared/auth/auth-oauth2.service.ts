...
export class AuthServerProvider {
    ...
    login(credentials): Observable<any> {
        const data = 'username=' +  encodeURIComponent(credentials.username) + '&password=' +
            ...
            'client_secret=my-secret-token-to-change-in-production&client_id=testprojectapp';
        const headers = new Headers ({
            'Content-Type': 'application/x-www-form-urlencoded',
            'Accept': 'application/json',
            'Authorization': 'Basic ' + this.base64.encode('testprojectapp' + ':' + 'my-secret-token-to-change-in-production')
        });
